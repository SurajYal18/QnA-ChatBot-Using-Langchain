...
def save_session_metadata(mongoDatabase, sessionId, folderId, folderName):
    """
    Stores or updates metadata for chat session:
    - last active time
    - selected folder
    """
    try:
...
def load_chat_history_summary(sessionHistory):
    """
    Scans all MongoDB-stored chat messages and returns:
    - last stored summary (without [SUMMARY] tag)
    If no summary exists → returns empty string.
    """
    try:
        
        for msg in reversed(sessionHistory.messages):
        ...
        print(f"  Error reading chat history summary: {e}")
        return ""
...
def chat_loop(mongoDatabase, dbRecord, vectorStore, llm, appConfig, sessionId, useWebSearch):
    """
    ...
    try:
        ...
        if ragChain is None:
            print("Failed to initialize the RAG chain.")
            return
        ...
        print(f"Web Search Enabled: {useWebSearch}")
        ...
        chat_summary = load_chat_history_summary(sessionHistory)
        ...
        while True:
            ...
            try:
                ...
                if retriever:
                    try:
                        retrieved_docs = retriever.invoke(userQuery)
                    except:
                        pass
                ...
                rawResponse = ragChain.invoke({
                    "input": str(userQuery),
                    "chat_history_summary": str(chat_summary)
                })

                response_text = ""
                try:
                   
                    if hasattr(rawResponse, 'content'):
                        response_text = rawResponse.content
                    elif isinstance(rawResponse, dict):
                        response_text = rawResponse.get('content', str(rawResponse))
                    else:
                        response_text = str(rawResponse)
                ...
                if result.get("final_answer") == "RAG_CANNOT_ANSWER":
                    if useWebSearch:
                        ...
                        # Display web sources with titles and URLs
                        if webSources:
                            print("\n📚 Sources (Web):")
                            for i, src in enumerate(webSources[:5], 1):
                            ...
                            print("\n📚 Sources: Web search (no specific URLs available)")

                        if chat_summary:
                            chat_summary = f"{chat_summary} | Web Search Q&A: Asked about '{userQuery[:50]}' - answered via web search."
                        else:
                            chat_summary = f"Web Search Q&A: Asked about '{userQuery[:50]}' - answered via web search."

                        sessionHistory.add_user_message(str(userQuery))
                        sessionHistory.add_ai_message(str(webAnswer))
                        
                        sessionHistory.add_message(AIMessage(content=f"[SUMMARY]{chat_summary}"))
                        save_session_metadata(mongoDatabase, sessionId, dbRecord["_id"], dbRecord["FolderName"])
                        continue
                    else:
                ...
                final_answer = str(result.get("final_answer", "No answer provided."))
                new_history_summary = str(result.get("new_history_summary", ""))
                
                print("\n Answer:")
                print(final_answer)
                
                if retrieved_docs:
                    doc_sources = format_document_sources(retrieved_docs)
                    if doc_sources:
                        print(f"\n📚 Sources (from {dbRecord['FolderName']}):")
                        for src in doc_sources:
                            print(f"   {src}")
                
                chat_summary = new_history_summary

                sessionHistory.add_user_message(str(userQuery))
                sessionHistory.add_ai_message(str(final_answer))
                
                if new_history_summary:
                    summary_msg = f"[SUMMARY]{new_history_summary}"
                    sessionHistory.add_message(AIMessage(content=summary_msg))
...
def resume_previous_session(mongoDatabase, embeddingModel, appConfig):
    """
    Shows list of previous sessions → user chooses → restores session.
    """
    try:
        ...
        chat_loop(mongoDatabase, folderRecord, vectorStore, llm, appConfig, chosenId, useWeb)
